...
-- Creates the heavy-lifting database functions:
--   1. current_snapshot_schema_version()  — version constant
--   2. set_subtree_visibility()           — bulk visibility updates
--   3. deep_copy_estimate()               — full deep-copy with FK remapping
--   4. create_estimate_snapshot()         — serialize estimate tree to JSONB
--   5. restore_estimate_snapshot()        — deserialize JSONB back to tables
--   6. create_estimate_from_snapshot()    — create NEW estimate from snapshot
--
-- Depends on:
--   20260409000001_security_foundation.sql  (is_staff(), get_user_role())
--   20260409000002_enums_and_extensions.sql (all enums, ltree)
--   20260409000003_reference_and_core_tables.sql (projects, estimates,
--     estimate_nodes, node_item_details, node_assembly_details, node_notes)
--   20260409000005_catalog_options_vendors.sql (option_groups, option_alternatives,
--     node_option_memberships, option_sets, option_set_selections,
--     broad_options, broad_option_overrides, option_set_broad_selections, vendors)
--   20260409000006_client_sharing_tables.sql (estimate_snapshots)


-- ############################################################
--  1. current_snapshot_schema_version()
-- ############################################################
-- IMMUTABLE version constant. Update when JSONB snapshot schema changes.
-- Used by create/restore functions to tag and validate snapshots.
...
COMMENT ON FUNCTION public.current_snapshot_schema_version() IS
  'Returns the current snapshot JSONB schema version. '
  'Increment when adding/removing/renaming columns in serialized tables.';

GRANT EXECUTE ON FUNCTION public.current_snapshot_schema_version() TO authenticated;
REVOKE EXECUTE ON FUNCTION public.current_snapshot_schema_version() FROM anon, public;


-- ############################################################
--  2. set_subtree_visibility()
-- ############################################################
-- Updates client_visibility for a node and ALL its ltree descendants.
-- Uses the path <@ operator for efficient subtree matching.
...
COMMENT ON FUNCTION public.set_subtree_visibility(UUID, public.client_visibility) IS
  'Sets client_visibility for a node and all its ltree descendants. '
...
REVOKE EXECUTE ON FUNCTION public.set_subtree_visibility(UUID, public.client_visibility) FROM anon, public;


-- ############################################################
--  3. deep_copy_estimate()
-- ############################################################
-- Deep-copies an entire estimate (all nodes, details, options,
-- memberships, option sets) into a new estimate with fully remapped IDs.
--
-- Uses temp-table ID mapping for O(1) lookups.
-- Bypasses history/path triggers during copy via SET LOCAL app.is_snapshot_copy.
...
COMMENT ON FUNCTION public.deep_copy_estimate(UUID, TEXT, UUID) IS
  'Deep-copies an entire estimate with full FK remapping across all related tables. '
  'Uses temp-table ID mapping for set-based operations. Bypasses triggers '
  'during copy via SET LOCAL app.is_snapshot_copy. Returns new estimate UUID.';

GRANT EXECUTE ON FUNCTION public.deep_copy_estimate(UUID, TEXT, UUID) TO authenticated;
REVOKE EXECUTE ON FUNCTION public.deep_copy_estimate(UUID, TEXT, UUID) FROM anon, public;


-- ############################################################
--  4. create_estimate_snapshot()
-- ############################################################
-- Serializes the full estimate tree into a JSONB snapshot stored
-- in estimate_snapshots. Captures all tables associated with the estimate.
...
COMMENT ON FUNCTION public.create_estimate_snapshot(UUID, VARCHAR, public.snapshot_type, UUID) IS
  'Serializes a full estimate tree (nodes, details, notes, options) into a JSONB '
  'snapshot stored in estimate_snapshots. Returns the new snapshot UUID.';

GRANT EXECUTE ON FUNCTION public.create_estimate_snapshot(UUID, VARCHAR, public.snapshot_type, UUID) TO authenticated;
REVOKE EXECUTE ON FUNCTION public.create_estimate_snapshot(UUID, VARCHAR, public.snapshot_type, UUID) FROM anon, public;


-- ############################################################
--  5. restore_estimate_snapshot()
-- ############################################################
-- Auto-saves current state as checkpoint, then deserializes snapshot
-- JSONB back into the estimate's production tables.
-- Uses advisory lock to prevent concurrent restores.
...
COMMENT ON FUNCTION public.restore_estimate_snapshot(UUID, UUID, BOOLEAN) IS
  'Restores an estimate from a JSONB snapshot. Auto-saves current state as '
  'checkpoint first. Uses advisory lock to prevent concurrent restores. '
  'Status guards: draft=allowed, preliminary=allowed, active=force required, '
  'complete=blocked. Returns the auto-checkpoint snapshot ID.';

GRANT EXECUTE ON FUNCTION public.restore_estimate_snapshot(UUID, UUID, BOOLEAN) TO authenticated;
REVOKE EXECUTE ON FUNCTION public.restore_estimate_snapshot(UUID, UUID, BOOLEAN) FROM anon, public;


-- ############################################################
--  6. create_estimate_from_snapshot()
-- ############################################################
-- Creates a NEW estimate from a snapshot (does not overwrite existing).
-- Similar to restore but targets a new estimate_id.
...
DECLARE
  ...
  v_source_vg_id        UUID;
  v_rec                 JSONB;
  -- Temp mapping tables for ID remapping (snapshot IDs -> new IDs)
  -- We need remapping because the new estimate has a different estimate_id
  -- and we want fresh node IDs to avoid any collision risk.
BEGIN
  -- ── 1. Load and validate snapshot ────────────────────────────
...
COMMENT ON FUNCTION public.create_estimate_from_snapshot(UUID, TEXT, UUID) IS
  'Creates a NEW estimate from a JSONB snapshot. Unlike restore, this does not '
  'overwrite any existing estimate. Uses temp-table ID remapping for all FKs. '
  'New estimate starts as draft. Returns the new estimate UUID.';

GRANT EXECUTE ON FUNCTION public.create_estimate_from_snapshot(UUID, TEXT, UUID) TO authenticated;
REVOKE EXECUTE ON FUNCTION public.create_estimate_from_snapshot(UUID, TEXT, UUID) FROM anon, public;
